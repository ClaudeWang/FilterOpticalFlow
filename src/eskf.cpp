...
void ESKF::prediction(float dt, VectorXf imu){
    ...
    v = v + (wRb * (acc - _nom_state.segment(10, 13)) + Vector3f{0, 0, -9.8}) * dt;
...
void ESKF::measurement(MatrixXf flow, MatrixXf K, MatrixXf depth, MatrixXf position){
    ...
    MatrixXf Kp(15, 3);
    Kp = _cov * C.tranpose() / (C * _cov * C.tranpose() + _R);
    ...
    Vector3f d_omg = 0.5 * _err_state(6, 9);
    Vector4f dq;

    if(d_omg.norm() > 1){
        dq = {1, d_omg[0], d_omg[1], d_omg[2]} / sqrt(1 + d_omg.dot(d_omg)) ;
    } else {
        dq = {sqrt(1 - d_omg.dot(d_omg)), d_omg[0], d_omg[1], d_omg[2]};
    }

    Vector4f q_true = quatMultiply(dq, _nom_state(3, 7));
    Vector3f b_a_true = _nom_state.segment(10, 13) + _err_state(9, 12);
...
Matrix3f skew(Vector3f vec){
    Matrix3f s = {0, -vec[0], vec[1], vec[0], 0, -vec[2], -vec[1], vec[2], 0};
    return s;
}