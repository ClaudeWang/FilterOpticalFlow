...
namespace filter_optical_flow {
  Undistorter::Undistorter(const ros::NodeHandle& nh,
    ...
    : nh_(nh),
      ...
      T_cam_imu_(cv::Mat::zeros(4, 4, CV_32F)){
    loadCamInfo(cam);
    initLookupTable();
  ...
  void Undistorter::loadCamInfo(const std::string& cam) {
    if (nh_.hasParam(cam)) {
      ...
      nh_.getParam(cam+"/projection_matrix", lines);
      if (lines.size() != 3 ||
	...
	for (int j = 0; j < lines[i].size(); j++) {
      ...
      nh_.getParam(cam+"/rectification_matrix", lines);
      if (lines.size() != 3 ||
	  lines.getType() != XmlRpc::XmlRpcValue::TypeArray) {
	...
	for (int j = 0; j < lines[i].size(); j++) {
      ...
      nh_.getParam(cam+"/T_cam_imu", lines);
      if (lines.size() != 4 ||
	  lines.getType() != XmlRpc::XmlRpcValue::TypeArray) {
	ROS_ERROR("invalid data in calib file cam imu transform");
	return;
      }
	...
	for (int j = 0; j < lines[i].size(); j++) {
	  if (lines[i][j].getType() != XmlRpc::XmlRpcValue::TypeDouble) {
	    ROS_ERROR_STREAM("bad value in calib file cam imu transform");
	  } else {
	    T_cam_imu_.at<float>(i, j) =
              static_cast<float>(static_cast<double>(lines[i][j]));
	  }
	}
      }
