...
namespace filter_optical_flow {
  ...
  public:
    ...
    inline std::string distortionModel() const {
      return distortion_model_;
    }

    inline cv::Mat P() const {
      return P_;
    }

    inline cv::Mat R() const {
      return R_;
    }

    inline cv::Mat distortion() const {
      return distortion_coeffs_;
    }

    inline cv::Mat T_cam_imu() const {
      return T_cam_imu_;
    }
   ...
   private:
    ...
    cv::Mat K_, P_, R_, distortion_coeffs_, T_cam_imu_;
  };
} // End namespace.
